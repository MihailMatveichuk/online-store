import { ICartProps, IPurchase } from '../types';
...
const CartsProduct = ({ onAdd, onDelete, product }: ICartProps) => {
  ...
  const currProduct: IPurchase[] = ordersStorage.filter(
    (el: IPurchase) => el.id === product.id
  );
  ...
  return (
    <CartStyledDiv>
      ...
      <ButtonDiv>
        <Button
          onClick={() =>
            countOfProduct < product.stock ? onAdd(product) : null
          }
